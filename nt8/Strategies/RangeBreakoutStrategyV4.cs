...
namespace NinjaTrader.NinjaScript.Strategies
{
    public class RangeBreakoutStrategyV4 : Strategy
    {
        ...
        /// <summary>False until initial SL/PT prices are set for the open position; mirrors Python carrying stop_price across bars.</summary>
        private bool _tradeStopsInitialized;
        ...
        private void LoadInstrumentConfig()
        {
            ...
            // Instrument preset above is authoritative (matches strategy_configs.py). Do not overwrite
            // range/trade/close times from UI fields — hidden defaults are MNQ and would desync CL/MGC/YM.
        ...
        private void ProcessPosition(DateTime barTime)
        {
            ...
            if (!_tradeStopsInitialized)
            {
                ...
                _tradeStopsInitialized = true;
            }

            // Same order as fast_engine.run_backtest: SL, PT, then update best, then BE/trail (stop carries bar to bar).
            if (_isLong)
            {
                if (Low[0] <= _stopPrice)
                {
